...
async def list_agents(user_id: UUID = Query(..., description="Authenticated user id")) -> list[AgentRead]:
    return AgentRepository().list_for_user(user_id)


@router.get("/{agent_id}", response_model=AgentRead)
async def get_agent(agent_id: UUID) -> AgentRead:
